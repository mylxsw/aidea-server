...
// ChatGroupN is a ChatGroup object, all fields are nullable
type ChatGroupN struct {
	original       *chatGroupOriginal
...
func (inst *ChatGroupN) As(dst interface{}) error {
	return query.Copy(inst, dst)
}

// SetModel set model for ChatGroup
func (inst *ChatGroupN) SetModel(chatGroupModel *ChatGroupModel) {
	inst.chatGroupModel = chatGroupModel
}

// chatGroupOriginal is an object which stores original ChatGroup from database
type chatGroupOriginal struct {
	Id        null.Int
	UserId    null.Int
	Name      null.String
	CreatedAt null.Time
...
func (inst *ChatGroupN) Staled(onlyFields ...string) bool {
	if inst.original == nil {
		inst.original = &chatGroupOriginal{}
	}
...
func (inst *ChatGroupN) StaledKV(onlyFields ...string) query.KV {
	kv := make(query.KV, 0)
...
func (inst *ChatGroupN) Save(ctx context.Context, onlyFields ...string) error {
	if inst.chatGroupModel == nil {
		return query.ErrModelNotSet
	}

	id, _, err := inst.chatGroupModel.SaveOrUpdate(ctx, *inst, onlyFields...)
	if err != nil {
		return err
	}
...
// Delete remove a chat_group
func (inst *ChatGroupN) Delete(ctx context.Context) error {
	if inst.chatGroupModel == nil {
		return query.ErrModelNotSet
	}

	_, err := inst.chatGroupModel.DeleteById(ctx, inst.Id.Int64)
	if err != nil {
		return err
	}
...
func (inst *ChatGroupN) String() string {
	rs, _ := json.Marshal(inst)
	return string(rs)
}

type chatGroupScope struct {
	name  string
	apply func(builder query.Condition)
}

var chatGroupGlobalScopes = make([]chatGroupScope, 0)
var chatGroupLocalScopes = make([]chatGroupScope, 0)

// AddGlobalScopeForChatGroup assign a global scope to a model
func AddGlobalScopeForChatGroup(name string, apply func(builder query.Condition)) {
	chatGroupGlobalScopes = append(chatGroupGlobalScopes, chatGroupScope{name: name, apply: apply})
}

// AddLocalScopeForChatGroup assign a local scope to a model
func AddLocalScopeForChatGroup(name string, apply func(builder query.Condition)) {
	chatGroupLocalScopes = append(chatGroupLocalScopes, chatGroupScope{name: name, apply: apply})
}

func (m *ChatGroupModel) applyScope() query.Condition {
	scopeCond := query.ConditionBuilder()
	for _, g := range chatGroupGlobalScopes {
		if m.globalScopeEnabled(g.name) {
			g.apply(scopeCond)
		}
	}

	for _, s := range chatGroupLocalScopes {
...
func (m *ChatGroupModel) localScopeEnabled(name string) bool {
...
func (m *ChatGroupModel) globalScopeEnabled(name string) bool {
...
func (w ChatGroup) ToChatGroupN(allows ...string) ChatGroupN {
	if len(allows) == 0 {
		return ChatGroupN{
...
// As convert object to other type
// dst must be a pointer to struct
func (w ChatGroup) As(dst interface{}) error {
	return query.Copy(w, dst)
}

func (w *ChatGroupN) ToChatGroup() ChatGroup {
	return ChatGroup{
...
// ChatGroupModel is a model which encapsulates the operations of the object
...
var chatGroupTableName = "chat_group"

// ChatGroupTable return table name for ChatGroup
func ChatGroupTable() string {
	return chatGroupTableName
}

const (
	FieldChatGroupId        = "id"
	FieldChatGroupUserId    = "user_id"
	FieldChatGroupName      = "name"
	FieldChatGroupCreatedAt = "created_at"
	FieldChatGroupUpdatedAt = "updated_at"
)

// ChatGroupFields return all fields in ChatGroup model
func ChatGroupFields() []string {
	return []string{
		"id",
		"user_id",
		"name",
...
func SetChatGroupTable(tableName string) {
	chatGroupTableName = tableName
}

// NewChatGroupModel create a ChatGroupModel
func NewChatGroupModel(db query.Database) *ChatGroupModel {
	return &ChatGroupModel{
		db:                  query.NewDatabaseWrap(db),
		tableName:           chatGroupTableName,
...
func (m *ChatGroupModel) GetDB() query.Database {
	return m.db.GetDB()
}

func (m *ChatGroupModel) clone() *ChatGroupModel {
	return &ChatGroupModel{
		db:                  m.db,
...
func (m *ChatGroupModel) WithoutGlobalScopes(names ...string) *ChatGroupModel {
	mc := m.clone()
...
func (m *ChatGroupModel) WithLocalScopes(names ...string) *ChatGroupModel {
	mc := m.clone()
...
func (m *ChatGroupModel) Condition(builder query.SQLBuilder) *ChatGroupModel {
	mm := m.clone()
...
func (m *ChatGroupModel) Find(ctx context.Context, id int64) (*ChatGroupN, error) {
	return m.First(ctx, m.query.Where("id", "=", id))
}

// Exists return whether the records exists for a given query
func (m *ChatGroupModel) Exists(ctx context.Context, builders ...query.SQLBuilder) (bool, error) {
	count, err := m.Count(ctx, builders...)
	return count > 0, err
}

// Count return model count for a given query
func (m *ChatGroupModel) Count(ctx context.Context, builders ...query.SQLBuilder) (int64, error) {
...
func (m *ChatGroupModel) Paginate(ctx context.Context, page int64, perPage int64, builders ...query.SQLBuilder) ([]ChatGroupN, query.PaginateMeta, error) {
	if page <= 0 {
		page = 1
	}
...
func (m *ChatGroupModel) Get(ctx context.Context, builders ...query.SQLBuilder) ([]ChatGroupN, error) {
	...
	var createScanVar = func(fields []query.Expr) (*ChatGroupN, []interface{}) {
		var chatGroupVar ChatGroupN
		...
		for _, f := range fields {
			...
			case "id":
				scanFields = append(scanFields, &chatGroupVar.Id)
			case "user_id":
				scanFields = append(scanFields, &chatGroupVar.UserId)
			case "name":
				scanFields = append(scanFields, &chatGroupVar.Name)
			case "created_at":
				scanFields = append(scanFields, &chatGroupVar.CreatedAt)
			case "updated_at":
				scanFields = append(scanFields, &chatGroupVar.UpdatedAt)
			}
		}

		return &chatGroupVar, scanFields
	}
	...
	chatGroups := make([]ChatGroupN, 0)
	for rows.Next() {
		chatGroupReal, scanFields := createScanVar(fields)
		if err := rows.Scan(scanFields...); err != nil {
			return nil, err
		}

		chatGroupReal.original = &chatGroupOriginal{}
		_ = query.Copy(chatGroupReal, chatGroupReal.original)

		chatGroupReal.SetModel(m)
		chatGroups = append(chatGroups, *chatGroupReal)
	}
...
func (m *ChatGroupModel) First(ctx context.Context, builders ...query.SQLBuilder) (*ChatGroupN, error) {
...
// Create save a new chat_group to database
func (m *ChatGroupModel) Create(ctx context.Context, kv query.KV) (int64, error) {
...
// SaveAll save all chat_groups to database
func (m *ChatGroupModel) SaveAll(ctx context.Context, chatGroups []ChatGroupN) ([]int64, error) {
	ids := make([]int64, 0)
	for _, chatGroup := range chatGroups {
		id, err := m.Save(ctx, chatGroup)
		if err != nil {
			return ids, err
		}
...
// Save save a chat_group to database
func (m *ChatGroupModel) Save(ctx context.Context, chatGroup ChatGroupN, onlyFields ...string) (int64, error) {
	return m.Create(ctx, chatGroup.StaledKV(onlyFields...))
}

// SaveOrUpdate save a new chat_group or update it when it has a id > 0
func (m *ChatGroupModel) SaveOrUpdate(ctx context.Context, chatGroup ChatGroupN, onlyFields ...string) (id int64, updated bool, err error) {
	if chatGroup.Id.Int64 > 0 {
		_, _err := m.UpdateById(ctx, chatGroup.Id.Int64, chatGroup, onlyFields...)
		return chatGroup.Id.Int64, true, _err
	}

	_id, _err := m.Save(ctx, chatGroup, onlyFields...)
	return _id, false, _err
}

// UpdateFields update kv for a given query
func (m *ChatGroupModel) UpdateFields(ctx context.Context, kv query.KV, builders ...query.SQLBuilder) (int64, error) {
	if len(kv) == 0 {
		return 0, nil
	}
...
func (m *ChatGroupModel) Update(ctx context.Context, builder query.SQLBuilder, chatGroup ChatGroupN, onlyFields ...string) (int64, error) {
	return m.UpdateFields(ctx, chatGroup.StaledKV(onlyFields...), builder)
}

// UpdateById update a model by id
func (m *ChatGroupModel) UpdateById(ctx context.Context, id int64, chatGroup ChatGroupN, onlyFields ...string) (int64, error) {
	return m.Condition(query.Builder().Where("id", "=", id)).UpdateFields(ctx, chatGroup.StaledKV(onlyFields...))
}

// Delete remove a model
func (m *ChatGroupModel) Delete(ctx context.Context, builders ...query.SQLBuilder) (int64, error) {
...
func (m *ChatGroupModel) DeleteById(ctx context.Context, id int64) (int64, error) {
